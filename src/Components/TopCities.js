...
import { Link } from "react-router-dom"; // bu import yapısının çalışması için "npm install react-router-dom" terminal de intall ediyoruz.
import Slider from "react-slick";
import { Left, Right } from "./Arrows"; // bu kısım arrow isimli komponent den data çekiyor.
    ...
    return (
      <div>
        <div className="top-cities-wrapper">
          ...
          <hr // hr sayfaya yatay çizgi eklememizi sağlar syle içindeki kodlar çizginin görünümü gösteriyor.
            style={{
...
// aşağıdaki yapı bir ögeyi ekranda yatay ve dikey kaydırmamıza yarıyor.
...
class CitySlider extends React.Component {
  render() {
    var settings = {
      ...
      speed: 500,
      slidesToShow: 5,
      slidesToScroll: 5, // her bir kaydırmamızda kaç slaytın geçeceğini söylüyor
      nextArrow: <Right />, // left ve right için özel komponentler oluşturulmuştur.
      prevArrow: <Left />,
    };
    // aşağıda top şehirleri bir döngüye sokuyoruz. Böylelikle her bir şehri slayt şeklinde görebiliyoruz.İçine link yapısını ekledik. Örneğin ben New*orleans merak ettim üstüne tıklayarak içerik hakkında bilgi sahibi olabiliyorum yani linki bağlantı görevi görüyor.
    return (
      <Slider {...settings}>
        {topCitiesData &&
